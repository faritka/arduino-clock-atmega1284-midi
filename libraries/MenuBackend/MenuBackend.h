...
|| @contribution   Farit Nabiullin http://hobby.farit.ru
||
|| @description
...
#ifndef __MENUBACKEND_H
#define __MENUBACKEND_H

#include "MenuInterface.h"

class MenuBackend; //forward declaration of the menu backend
class MenuInterface; //forward declaration of the menu interface class
...
class MenuItem
{
  public:  
    ...
    */
    ...
    || @parameter shortKey the mnemonic 'shortkey'
    */
    MenuItem(MenuBackend *mb, const char* itemName, unsigned char lvl = 0, int val = -1, char shortKey = '\0') 
        : name(itemName), shortkey(shortKey)
    {
        registerBackend(mb);
        before = right = after = left = 0;
        value = val;
    ...
    */
    ...
    || @return bool
    */
    friend bool operator==(const MenuItem &item1, const MenuItem &item2)
    {
        return (&item1 == &item2);      
    }
    friend bool operator==(const MenuItem &item1, const MenuItem *item2)
...
{
  public:
    MenuBackend(MenuInterface *newMenu) : root("")
    {
        current = &root;
        menu    = newMenu;
    }
